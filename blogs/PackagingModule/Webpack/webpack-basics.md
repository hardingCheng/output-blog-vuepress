...
![](https://output66.oss-cn-beijing.aliyuncs.com/img/20211220161228.png)
- 方案二：使用webpack-bundle-analyzer工具
    - 直观查看包大小的工具是webpack-bundle-analyzer。
    - 第一步，我们可以直接安装这个工具：
        - `npm install webpack-bundle-analyzer -D`
    - 第二步，我们可以在webpack配置中使用该插件：
        - ![](https://output66.oss-cn-beijing.aliyuncs.com/img/20211220164232.png)
...
## Webpack的启动流程
![](https://output66.oss-cn-beijing.aliyuncs.com/img/20211220164955.png)
## Webpack源码阅读
### Webpack源码流程
- 第一步：下载webpack的源码
...
- 第五步：编写启动的文件build.js
### 创建Compiler
![](https://output66.oss-cn-beijing.aliyuncs.com/img/20211220193202.png)
### Compiler中run方法执行的Hook
![](https://output66.oss-cn-beijing.aliyuncs.com/img/20211220193404.png)
### Compilation对Module的处理
![](https://output66.oss-cn-beijing.aliyuncs.com/img/20211220193812.png)
### module的build阶段
![](https://output66.oss-cn-beijing.aliyuncs.com/img/20211220194003.png)
### 输出asset阶段
![](https://output66.oss-cn-beijing.aliyuncs.com/img/20211220194050.png)
### Compiler和Compilation的区别
- Compiler和Compilation的区别
    - Compiler在webpack构建的之初就会创建的一个对象, 并且在webpack的整个生命周期都会存在(before - run - beforeCompiler - compile - make - finishMake - afterCompiler - done)
    - 只要是做webpack的编译, 都会先创建一个Compiler

    - Compilation是到准备编译模块(比如main.js), 才会创建Compilation对象
    - 主要是存在于 compile（之后） - make（之后） 阶段主要使用的对象
    - watch -> 源代码发生改变就需要重新编译模块
    - Compiler可以继续使用(如果我修改webpack的配置, 那么需要重新执行run run build)
    - Compilation需要创建一个新的Compilation对象,修改源代码就会重新创建一个对象
### Plugin执行的顺序
