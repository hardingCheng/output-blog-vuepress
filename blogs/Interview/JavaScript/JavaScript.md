...
### CSS 和 JS 为什么会放在头部或者底部？
    - 因为页面在加载时，浏览器会识别该文档 CSS，并行下载，不会停止对当前文档的加载。放在头部可以保证在加载 html 生成 DOM tr·ee 的时候，就可以同时对 DOM tree 进行渲染，可以防止闪跳，白屏或者布局混乱。
...
当一个数据不再需要使用时，我们最好通 过将其值设置为 null 来释放其引用，这个做法叫做解除引用。不过解除一个值的引用并不意味着自动回收改值所占用的内存。解除引用的真正作用是让值脱离执行环境，以便垃圾收集器在下次运行时将其回收。解除引用还有助于消除有可能出现的循环引用的情况。这一做法适用于大多数全局变量和全局对象的属性，局部变量会在它们离开执行环境时(函数执行完时)自动被解除引用。
...
闭包也是一个对象。闭包就是每次调用外层函数时，临时创建的函数作用域对象。
...
- 可达的标准就是从根出发是否能够被找到
- js中的根就可以理解为是全局变量对象
![](https://output66.oss-cn-beijing.aliyuncs.com/img/20220110112449.png)
#### GC里的垃圾
- 程序中不再需要使用的对象
- 程序中不能再访问到的对象
### GC定义与作用
...
changeName();
```
![](media/16418089190119.jpg)
![](https://output66.oss-cn-beijing.aliyuncs.com/img/20220110180401.png)

在函数执行完毕之后，函数的声明周期结束，那么现在，从 Window 对象 出发， obj1 和 obj2 都会被垃圾收集器标记为不可抵达，这样子的情况下，互相引用的情况也会迎刃而解。

**标记整理算法**
![](https://output66.oss-cn-beijing.aliyuncs.com/img/20220110182402.png)

![](https://output66.oss-cn-beijing.aliyuncs.com/img/20220110182500.png)
### V8引擎
- V8是一款主流的js执行引擎
- V8采用即时编译
- V8内存设限
#### V8垃圾回收策略
![](https://output66.oss-cn-beijing.aliyuncs.com/img/20220110183118.png)
...
- 小空间用于存储新生代对象
- 新生代指的是存活时间较短的对象
![](https://output66.oss-cn-beijing.aliyuncs.com/img/20220110183414.png)
#### 新生代
新生代中通常只支持1~8M的容量，所以主要**存放生存时间较短的对象**。
新生代中使用`复制算法+标记整理`，将新生代空间分为两个等大小区域：对象区域（使用空间为From）和空闲区域(空闲空间To)。如图：
...
![](https://output66.oss-cn-beijing.aliyuncs.com/img/20220110184540.png)
